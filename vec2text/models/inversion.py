...
# Creates a BaseInversionModel which MLPInversionModel and AttentionInversionModel can inherit from
class BaseInversionModel(PreTrainedModel):
...
class MLPInversionModel(BaseInversionModel):
    def __init__(self, config: transformers.PretrainedConfig):
        super().__init__(config)
        
        # Initialize the MLP-based embedding transform
        encoder_hidden_dim = self.encoder_decoder.config.hidden_size
        self.bottleneck_dim = self.embedder.config.hidden_size if not config.embedder_model_api else 1536
        self.embedding_transform = nn.Sequential(
            nn.Linear(self.bottleneck_dim, self.bottleneck_dim),
            nn.Dropout(self.encoder_decoder.config.dropout_rate),
            nn.GELU(),
            nn.Linear(self.bottleneck_dim, encoder_hidden_dim * self.num_repeat_tokens),
        )

# Define the AttentionMechanism class used in the AttentionInversionModel
class AttentionMechanism(nn.Module):
    def __init__(self, embed_size, num_heads, output_dim):
        super(AttentionMechanism, self).__init__()
        self.attention = nn.MultiheadAttention(embed_size, num_heads)
        self.fc = nn.Linear(embed_size, output_dim)  # Final linear layer to match the output dimensions
        
    def forward(self, x):
        attn_output, _ = self.attention(x, x, x)
        return self.fc(attn_output)

class AttentionInversionModel(BaseInversionModel):
    def __init__(self, config: transformers.PretrainedConfig):
        super().__init__(config)
        
        # Initialize the Attention-based embedding transform
        encoder_hidden_dim = self.encoder_decoder.config.hidden_size
        self.bottleneck_dim = self.embedder.config.hidden_size if not config.embedder_model_api else 1536
        self.embedding_transform = AttentionMechanism(
            embed_size=self.bottleneck_dim,
            num_heads=48,  # Adjustable number of heads
            output_dim=encoder_hidden_dim * self.num_repeat_tokens
        )
